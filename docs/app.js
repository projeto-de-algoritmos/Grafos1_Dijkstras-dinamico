...
let EDGE_COUNT = 2; //conexoes
let NODE_COUNT = 3; //nodes
...
function main() {
  ...
  for (let i = 0; i < path.length - 1; i++) {
    // seleciona edge conectando os nos,
    // edge: [node 1 id][node 2 id],
...
function css() {
  ...
  cy.style(cytoscape.stylesheet()
    ...
    .selector('#start').css({ 'background-color': '#003cc8' })
    .selector('#finish').css({ 'background-color': '#ff0000' }));
}